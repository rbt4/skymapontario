(() => {
  ...
  const MODES = {
    rain:{label:'Rain',timeline:true,source:'ECCC radar, extrapolation and HRDPS'},
    storm:{label:'Storms',timeline:true,context:{layer:'Lightning_2.5km_Density',style:'Lightning'},source:'Radar with ECCC lightning density'},
    smoke:{label:'Smoke',layer:'RAQDPS.Sfc_PM2.5-WildfireSmokePlume',style:'',ahead:6,kind:'guidance',source:'RAQDPS wildfire smoke guidance',legend:{title:'Smoke',stops:'#f3e6b8,#e8b06a,#d1705c,#8f4757',low:'Light',high:'Dense'}},
    air:{label:'Air quality',layer:'AQHI-OBS',style:'default',ahead:0,kind:'observed',source:'Observed AQHI stations',legend:{title:'AQHI',stops:'#5ac8d8,#ffcf85,#ff9aa0,#b06a9a',low:'1 Low',high:'10+ Very high'}},
    temp:{label:'Temperature',layer:'HRDPS.CONTINENTAL_TT',style:'',ahead:2,kind:'guidance',source:'HRDPS 2.5 km temperature',legend:{title:'Temperature',stops:'#5b8dff,#6fdcff,#cdf283,#ffcf85,#ff846b',low:'Cold',high:'Warm'}}
  };
  const MODELS = [
  ...
  function geometEndpoints() { return IS_NATIVE?[NATIVE_GEOMET,GEOMET]:[GEOMET]; }
  async function geometFetch(params,{timeout=12000}={}) {
    const query=params instanceof URLSearchParams?params.toString():String(params); let lastError=null;
    for(const endpoint of geometEndpoints()){
      const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),timeout);
      try{const response=await fetch(`${endpoint}?${query}`,{cache:'no-store',signal:controller.signal});if(response.ok)return response;lastError=new Error(`GeoMet ${response.status}`);}
      catch(error){lastError=error;}finally{clearTimeout(timer);}
    }
    throw lastError||new Error('GeoMet unavailable');
  }
  async function geometCapabilities() {
    ...
    capabilitiesCache.promise=geometFetch('service=WMS&request=GetCapabilities&version=1.3.0&lang=en',{timeout:20000}).then(async response=>{
  ...
  async function buildFrames() {
    ...
    renderTimeline();
    if(MODES[state.mode].timeline)showFrame(state.frameIndex,false); else void setMode(state.mode);
  ...
  function showFrame(index,pan=true) {
    if(!MODES[state.mode].timeline)leaveStaticMode();
  ...
  function nearestTime(times,target) {
    let best=null,delta=Infinity; times.forEach(value=>{const d=Math.abs(new Date(value).getTime()-target);if(d<delta){delta=d;best=value;}}); return best;
  }
  function syncModeControls() {
    $$('[data-map-mode]').forEach(button=>{const active=button.dataset.mapMode===state.mode;button.classList.toggle('active',active);button.setAttribute('aria-pressed',String(active));});
    document.body.dataset.mode=state.mode;
  }
  function renderLegend() {
    const el=$('#map-legend'); if(!el)return; const legend=MODES[state.mode].legend;
    if(!legend){el.dataset.kind='rain';el.innerHTML=renderLegend.rain||el.innerHTML;return;}
    renderLegend.rain=renderLegend.rain||(el.dataset.kind==='rain'?el.innerHTML:'');
    el.dataset.kind='gradient';
    el.innerHTML=`<span class="legend-gradient"><b>${esc(legend.title)}</b><i style="background:linear-gradient(90deg,${esc(legend.stops)})"></i><small><em>${esc(legend.low)}</em><em>${esc(legend.high)}</em></small></span>`;
  }
  function updateContextLayer() {
    const context=MODES[state.mode].context;
    if(!context){if(state.contextOverlay){try{state.map.removeLayer(state.contextOverlay)}catch(_){} state.contextOverlay=null;}return;}
    if(state.contextOverlay)return;
    state.contextOverlay=L.tileLayer.wms(GEOMET,{layers:context.layer,styles:context.style,format:'image/png',transparent:true,version:'1.3.0',pane:'weather',opacity:.9,className:'weather-context'}).addTo(state.map);
  }
  function removeStaticOverlay() {
    if(state.staticOverlay){try{state.map.removeLayer(state.staticOverlay)}catch(_){} state.staticOverlay=null;}
  }
  function leaveStaticMode() {
    state.modeToken++; removeStaticOverlay(); state.mode='rain'; syncModeControls(); renderLegend(); updateContextLayer();
  }
  async function setMode(mode) {
    const config=MODES[mode]; if(!config||!state.map)return;
    const token=++state.modeToken; state.mode=mode; syncModeControls(); renderLegend(); updateContextLayer();
    if(config.timeline){removeStaticOverlay();if(state.frames.length)showFrame(state.frameIndex,false);return;}
    stop(); if(state.overlay){try{state.map.removeLayer(state.overlay)}catch(_){} state.overlay=null;}
    const phase=$('#source-phase'); phase.dataset.phase=config.kind;
    text('#phase-kicker',config.kind==='observed'?'OBSERVED':'GUIDANCE'); text('#phase-title',config.source);
    text('#time-kind',config.label.toUpperCase()); text('#time-label','Loading'); text('#time-detail','Single official layer · no playback');
    try{
      const meta=await layerTimes(config.layer); if(token!==state.modeToken)return;
      const time=nearestTime(meta.times,Date.now()+(config.ahead||0)*3600000);
      removeStaticOverlay();
      const opts={layers:config.layer,styles:config.style||'',format:'image/png',transparent:true,version:'1.3.0',pane:'weather',opacity:mode==='temp'?.66:.78,className:`weather-${config.kind}`};
      if(time)opts.time=time; if(meta.reference)opts.reference_time=meta.reference;
      state.staticOverlay=L.tileLayer.wms(GEOMET,opts).addTo(state.map);
      text('#time-label',time?fmtTime(time):'LATEST');
      text('#time-detail',config.kind==='observed'?'Latest official observations':`About ${config.ahead} h ahead · model guidance`);
    }catch(_){
      if(token!==state.modeToken)return;
      text('#time-label','Delayed'); showToast(`${config.label} layer is delayed. Rain remains available.`);
    }
  }
  ...
  function plausibleFeatureValue(properties) {
    ...
    const candidate=finite(preferred?.[1]??fallback?.[1]); return candidate==null?null:Math.max(0,candidate);
  }
  const featureCache=new Map();
  // Returns a number, null (layer answered with no value) or undefined (layer did not answer).
  async function featureValue(frame,point=state.place) {
    if(!frame?.layer||!frame.time)return undefined;
    const lat=Number(point.lat),lon=Number(point.lon); if(!Number.isFinite(lat)||!Number.isFinite(lon))return undefined;
    const key=`${frame.layer}|${frame.time}|${frame.reference||''}|${lat.toFixed(4)}|${lon.toFixed(4)}`;
    const cached=featureCache.get(key); if(cached&&Date.now()-cached.savedAt<5*60000)return cached.value;
    const d=.08; const params=new URLSearchParams({
      ...
      srs:'EPSG:4326',bbox:`${lon-d},${lat-d},${lon+d},${lat+d}`,width:'101',height:'101',x:'50',y:'50',
      info_format:'application/json',feature_count:'1',time:frame.time
    });
    if(frame.reference)params.set('reference_time',frame.reference);
    try{
      const data=await (await geometFetch(params,{timeout:9000})).json();
      const value=plausibleFeatureValue(data?.features?.[0]?.properties);
      featureCache.set(key,{value,savedAt:Date.now()}); if(featureCache.size>400)featureCache.delete(featureCache.keys().next().value);
      return value;
  ...
  async function queryPointValue(frame,live=false) {
    ...
    const rate=await featureValue(frame,state.place);
  ...
  async function setPlace(place,fromMap=false) {
    state.place={...state.place,...place}; savePlace(); placeMarker('loading'); text('#place-name',state.place.name); emit('place',{place:{...state.place}});
  ...
  function useLocation() {
    ...
    navigator.geolocation.getCurrentPosition(position=>setPlace({name:'My location',lat:position.coords.latitude,lon:position.coords.longitude,zoom:12}),error=>{
  ...
  function bind() {
    ...
    document.addEventListener('keydown',e=>{if(e.key==='Escape')toggleEvidence(false);});
    // Android back: close the innermost open surface before leaving the app.
    window.SkyMapBack=()=>{
      const dialog=[...document.querySelectorAll('dialog[open]')].pop(); if(dialog){dialog.close();return true;}
      if($('#evidence-drawer').classList.contains('open')){toggleEvidence(false);return true;}
      if(state.mode!=='rain'){setMode('rain');return true;}
      return false;
  ...
  // The small public surface used by the visit, conditions and native modules.
  // Forecast state stays owned here; modules read it and never mutate model rows.
  window.SkyMap={
    ...
    isNative:IS_NATIVE, models:MODELS.map(model=>({...model})),
    blendAt, layerTimes, featureValue, geometFetch, geometEndpoints, nearestTime,
    fmt, fmtTime, fmtDay, fmtDate, dateKey, esc, finite, clamp, showToast, setMode, openPlaceDialog,
    showTime(target){if(state.mode!=='rain'&&!MODES[state.mode].timeline)leaveStaticMode();stop();showFrame(nearestFrameIndex(target),true);document.querySelector('.map-stage')?.scrollIntoView({behavior:'smooth',block:'start'});}
  };

  async function init() {
    renderQuickPlaces(); bind(); initMap(); syncModeControls(); renderLegend(); text('#place-name',state.place.name);
    emit('ready',{place:{...state.place}});
    if('serviceWorker' in navigator&&!IS_NATIVE&&location.protocol==='https:')navigator.serviceWorker.register('sw.js',{updateViaCache:'none'}).catch(()=>{});
    await refreshAll();
