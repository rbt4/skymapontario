...
function parseModel(data,target) {
  ...
  const h=data.hourly;
  const code=finite(h.weather_code?.[i]);
  const p=finite(h.precipitation?.[i]), rain=finite(h.rain?.[i]), showers=finite(h.showers?.[i]), snow=finite(h.snowfall?.[i]);
  if([code,p,rain,showers,snow].every(v=>v==null)) return null;
  const amounts=[];
  if(p!=null)amounts.push(p);
  if(rain!=null||showers!=null)amounts.push((rain||0)+(showers||0));
  if(snow!=null)amounts.push(snow*.7);
  const precip=amounts.length?Math.max(0,...amounts):null;
  return { wet:(precip!=null&&precip>=WET_MM)||(code!=null&&PRECIP_CODES.has(code)), amount:precip==null?null:+precip.toFixed(3), probability:null, windowHours:1 };
}
...
function officialAt(feature,target) {
  ...
  if(prob==null&&!condition)return null;
...
function extractFeatureValue(data){
  ...
  const good=([k,v])=>!/(time|date|lat|lon|x|y|id|index)/i.test(k)&&v!==null&&v!==undefined&&v!==''&&Number.isFinite(Number(v));
...
async function climateTruth(loc,validAt,windowHours=1){
  ...
  const q=new URLSearchParams({f:'json',bbox:`${loc.lon-d},${loc.lat-d},${loc.lon+d},${loc.lat+d}`,datetime:`${iso(start)}/${iso(end)}`,limit:'250'});
  const data=await fetchJson(`${ECCC}/collections/climate-hourly/items?${q}`,2);
  const groups=new Map();
  for(const f of data.features||[]){
    const p=f.properties||{}, coords=f.geometry?.coordinates; if(!Array.isArray(coords))continue;
    const mm=finite(p.PRECIP_AMOUNT), desc=String(p.WEATHER_ENG_DESC||'');
    const hasDescription=desc&&desc.toUpperCase()!=='NA';
    if(mm==null&&!hasDescription)continue;
    const sid=String(p.CLIMATE_IDENTIFIER||p.STN_ID||p.STATION_NAME||''); if(!sid)continue;
  ...
  let amount=0, amountN=0, wet=false, evidence=0;
  for(const p of best.rows){
    const mm=finite(p.PRECIP_AMOUNT), desc=String(p.WEATHER_ENG_DESC||'');
    if(mm!=null){amount+=Math.max(0,mm);amountN++;evidence++;}
    if(desc&&desc.toUpperCase()!=='NA'){evidence++;if(WEATHER_WORDS.test(desc))wet=true;}
    if(mm!=null&&mm>=WET_MM)wet=true;
  }
...
async function collectLocation(loc, now, dimensions){
  const [best, official, ...modelData]=await Promise.all([
    fetchJson(bestMatchUrl(loc),2), getOfficial(loc).catch(()=>null),
...
function publicPayload(state){
  const metrics=Object.values(state.metrics).map(summarise), byLead={};
  for(const lead of LEADS){
    byLead[lead]=metrics.filter(m=>m.lead===lead).sort((a,b)=>{
      const rank=x=>x.samples<20?-1:(x.brier!=null?1-x.brier:(x.csi??0)); return rank(b)-rank(a);
    });
  }
  return {schema:2,generatedAt:state.updatedAt,runs:state.runs,pending:state.pending.length,locations:LOCATIONS.map(({id,name})=>({id,name})),leads:LEADS,note:'Experimental verification. Missing forecast/observation values are excluded rather than coerced to dry weather. Rankings are withheld until a source/lead pair has at least 20 verified samples.',byLead};
}
function report(publicData, verified, added){
  ...
  lines.push('Metrics: POD = probability of detection; FAR = false alarm ratio; CSI = critical success index. Brier is only calculated for probabilistic sources. Missing numeric data never counts as a dry forecast or dry observation.');
  return lines.join('\n');
}
async function selfTest(){
  if(finite(null)!==null||finite(undefined)!==null||finite('')!==null||finite(0)!==0)throw new Error('null-safety failed');
  const fake={hourly:{time:[1704067200],precipitation:[null],rain:[null],showers:[null],snowfall:[null],weather_code:[null]}};
  if(parseModel(fake,1704067200000)!==null)throw new Error('missing model hour became dry');
  ...
  console.log('✓ null-safe forecast backtester self-test passed');
}
async function main(){
  ...
  const [hrdps,reps]=await Promise.all([
    capability('HRDPS.CONTINENTAL.DIAG_PR_PT1H'), capability('REPS.DIAG.3_PRMM.ERGE1')
  ]);
  let added=0;
