...
async function loadTruth(c){const byLoc=new Map(LOCATIONS.map(x=>[x.id,new Map()]));let accepted=0,failed=0;for(const validAt of analysisTimes(c)){try{const payload=await fetchJson(truthUrl(validAt),3,55000);if(!describedCoverage&&process.argv.includes('--smoke')){describedCoverage=true;console.log(`ℹ RDPA CoverageJSON shape ${JSON.stringify(coverageShape(payload))}`);}const values=gridValues(payload,validAt);for(const [id,value]of values)byLoc.get(id)?.set(validAt,value);if(values.size>=6)accepted++;else failed++;}catch(e){failed++;console.warn(`⚠ RDPA ${iso(validAt)}: ${e.message}`);}await new Promise(r=>setTimeout(r,120));}const out=new Map([...byLoc].filter(([,series])=>series.size>=Math.max(2,Math.floor(accepted*.5))));console.log(`✓ RDPA truth grids: ${accepted} accepted · ${failed} failed · ${out.size}/${LOCATIONS.length} locations usable`);return out;}
