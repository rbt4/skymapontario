...
async function selfTest(){if(finite(null)!==null||finite('')!==null)throw new Error('null safety failed');const synthetic={type:'Coverage',domain:{axes:{t:{values:['2024-01-01T00:00:00Z']},y:{values:[42,44,46,48,50]},x:{values:[-96,-90,-84,-78,-72]}}},ranges:{APCP:{axisNames:['t','y','x'],shape:[1,5,5],values:Array.from({length:25},(_,i)=>i)},QUALITY:{axisNames:['t','y','x'],shape:[1,5,5],values:Array(25).fill(80)}}},sampleChunk={startMs:Date.parse('2024-01-01T00:00:00Z'),endMs:Date.parse('2024-01-01T00:00:00Z')},vals=gridSeries(synthetic,LOCATIONS[0],sampleChunk);if(vals.size!==1||vals.values().next().value?.amount==null)throw new Error('RDPA CoverageJSON point sampler failed');const map=new Map();for(let h=0;h<6;h++)map.set(h*3600000,1);if(sixHourTotal(map,5*3600000)!==6)throw new Error('6h forecast accumulator failed');const c={startMs:Date.parse('2024-01-01T00:00:00Z'),endMs:Date.parse('2024-01-02T00:00:00Z')};if(analysisTimes(c).length!==5)throw new Error('analysis clock failed');console.log('✓ CaPA/RDPA truth engine v2 self-test passed');}
